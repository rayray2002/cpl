...
void initwindow(int width, int height) {
	...
	if (NOPE == bgi_fast_mode) {  // called by initgraph ()
		if (!width || !height) {    // fullscreen
			bgi_maxx = mode.w - 1;
			bgi_maxy = mode.h - 1;
			window_flags = window_flags | SDL_WINDOW_FULLSCREEN_DESKTOP | SDL_WINDOW_ALLOW_HIGHDPI;
			fullscreen = YEAH;
		} else {
			bgi_maxx = width - 1;
			bgi_maxy = height - 1;
...
void line_copy(int x1, int y1, int x2, int y2) {
	...
	for (;;) {
		...
		else if ((line_patterns[bgi_line_style.linestyle] >> counter % 16) & 1)
			putpixel_copy(x1, y1, palette[bgi_fg_color]);
...
void readimagefile(const char *bitmapname, int x1, int y1, int x2, int y2) {
	...
	bgi_activepage[current_window] = (Uint32 *) cur_surface->pixels;
	refresh();
...
void setactivepage(int page) {
	...
	if (page > -1 && page < bgi_np) {
		bgi_activepage[current_window] = (Uint32 *) bgi_vpage[page]->pixels;
		bgi_ap = page;
	}
...
void setcurrentwindow(int id) {
	...
	SDL_GL_GetDrawableSize(bgi_window, &bgi_maxx, &bgi_maxy);
...
void setvisualpage(int page) {
	...
	if (page > -1 && page < bgi_np) {
		bgi_vp = page;
		bgi_visualpage[current_window] = (Uint32 *) bgi_vpage[bgi_vp]->pixels;
	}
...
void setwinoptions(const char *title, int x, int y, Uint32 flags) {
	if (strlen(title) > BGI_WINTITLE_LEN)
		fprintf(stderr, "BGI window title name too long.\n");
	else if (0 != strlen(title))
